...
function FetchData() {
    ...
    const handleSearch =  async (searchQuery) => {
        ...
        // const filterData = dataFetch.filter((item) => item.title.toLowerCase().includes(searchQuery.toLowerCase()));
        // setDataFetch(filterData);
        // console.log('handleSearch: ', searchQuery);
        try{
            setIsLoading(true);
            const response= await axios.get('https://api.jikan.moe/v4/anime/search',{
                params: {
                    q: searchQuery,
                }
            });
            ...
            {
                console.log('filter: ', responseData);
    ...
    useEffect(() => {
        ...
        const dataCheckInterval = setInterval(checkDataAvailability, 1000);
    ...
    const paginatedData  = dataFetch.slice(startIndex, endIndex);
  ...
  return (
    ...
    <section className="bg-white dark:bg-gray-900">
        <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6 ">
            <Pagination currentPage={currentPage} totalPage={Math.ceil(dataFetch.length / perPage)} onPageChange={handlePageChange} />  
        </div>
    </section>
    
    {modalVisible && (
                <Modal image={selectedImage} closeModal={() => setModalVisible(false)} />
            )}
